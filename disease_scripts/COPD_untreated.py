...
# This script gives patients COPD of varying severity does not treat it
# Arguments: patient - a patient name, level_severity - a COPD severity


def COPD_untreated(patient, level_severity):
    # Initialize the pulse engine, tell pulse where to send the log file, and also show the log on the console
    pulse = PulseEngine()
    pulse.set_log_filename("./test_results/XCOR/COPD_untreated_{}_{}.log".format(patient, level_severity))
    ...
    data_mgr.set_results_filename("./test_results/XCOR/COPD_untreated_{}_{}.csv".format(patient, level_severity))
