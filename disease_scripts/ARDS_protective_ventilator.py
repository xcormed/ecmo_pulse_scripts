...
# This script gives patients ARDS of varying severity and treats them with protective ventilation

# Arguments: patient - a patient name, severity_level - an ARDS severity, VoT - a tidal volume


def ARDS_ventilator_protective(patient, severity_level, VoT):
    # Initialize the pulse engine, tell pulse where to send the log file, and also show the log on the console
    pulse = PulseEngine()
    pulse.set_log_filename("./test_results/XCOR/ARDS_ventilator_{}_{}.log".format(patient, severity_level))
    ...
    data_mgr.set_results_filename("./test_results/XCOR/ARDS_ventilator_{}_{}.csv".format(patient, severity_level))
    ...
    ards.get_severity().set_value(severity_level)
...
#             processes.append(mp.Process(None, ARDS_ventilator_protective, args=(name, severity, VT[i])))
#
#     for p in processes:
#         p.start()
#
