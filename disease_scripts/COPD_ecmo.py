...
def COPD_ecmo(patient, level_severity):
    ...
    data_requests = [
        ...
        SEDataRequest.create_liquid_compartment_substance_request("VenaCava", "CarbonDioxide", "PartialPressure",
                                                                  unit=PressureUnit.mmHg),
        SEDataRequest.create_liquid_compartment_substance_request("VenaCava", "Oxygen", "PartialPressure",
                                                                  unit=PressureUnit.mmHg),
    ...
    estimate = max((results[list(results)[26]][-1] - 15) * 0.0321, 0)
    estimate_co2 = min(estimate, 0.0321)
    settings.get_substance_concentration("CarbonDioxide").get_concentration().set_value(
        results[list(results)[29]][-1] - estimate_co2, MassPerVolumeUnit.g_Per_dL)
    ...
    if results[list(results)[7]][-1] < 1.0 or results[list(results)[27]][-1] <= 300:
        settings.get_substance_concentration("Oxygen").get_concentration().set_value(
            results[list(results)[28]][-1]+0.00992, MassPerVolumeUnit.g_Per_dL)
