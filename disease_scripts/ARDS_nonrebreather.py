...
# This script gives patients ARDS of varying severity and treats them with a nonrebreather mask

# Arguments: patient - a patient name, severity - an ARDS severity


def ARDS_nonrebreather(patient, severity):
    # Initialize the pulse engine, tell pulse where to send the log file, and also show the log on the console
    pulse = PulseEngine()
    pulse.set_log_filename("./test_results/XCOR/ARDS_nonrebreather_{}_{}.log".format(patient, severity))
    ...
    data_mgr.set_results_filename("./test_results/XCOR/ARDS_nonrebreather_{}_{}.csv".format(patient, severity))
    ...
    ards.get_severity().set_value(severity)
    pulse.process_action(ards)

    # Define and process a supplemental oxygen action. Nonrebreather mask with 10 L/min flow rate.
    oxygen = SESupplementalOxygen()
    oxygen.set_comment("Patient receives supplemental oxygen ")
    oxygen.set_device(eDevice.NonRebreatherMask)
    oxygen.get_volume().set_value(400, VolumeUnit.L)
    oxygen.get_flow().set_value(10, VolumePerTimeUnit.L_Per_min)
