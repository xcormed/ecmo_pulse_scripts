...
# This script gives patients ARDS of varying severity and treats them with traditional ventilation
# Arguments: patient - a patient name, level_severity - an ARDS severity, VoT - a tidal volume


def ARDS_ventilator_traditional(patient, level_severity, VoT):
    # Initialize the pulse engine, tell pulse where to send the log file, and also show the log on the console
    pulse = PulseEngine()
    pulse.set_log_filename("./test_results/XCOR/ARDS_traditional_ventilator_{}_{}.log".format(patient, level_severity))
    ...
    data_mgr.set_results_filename("./test_results/XCOR/ARDS_traditional_ventilator_{}_{}.csv".format(patient, level_severity))
    ...
    ards.get_severity().set_value(level_severity)
...
#             processes.append(mp.Process(None, ARDS_ventilator_traditional, args=(name, severity, VT[i])))
#
#     for p in processes:
#         p.start()
#
