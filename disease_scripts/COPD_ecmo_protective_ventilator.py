...
def COPD_ecmo_protective_ventilator(patient, level_severity, VoT):
    ...
    data_mgr.set_results_filename("./test_results/XCOR/COPD_ecmo_protective_ventilator{}_{}.csv".format(patient,
                                                                                                         level_severity))
...
# Add a new thread for every patient at each severity, start each thread, and join them
    for i, name in enumerate(names):
        for severity in severities:
            processes.append(mp.Process(None, COPD_ecmo_protective_ventilator, args=(name, severity, VT[i])))
